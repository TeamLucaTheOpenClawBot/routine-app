...
- 주간 목표 달성 판정은 **지난(완료된) 주만 확정**. 이번 주는 항상 "진행 중".
  **기록이 하나도 없는 주는 판정하지 않는다** — 통계(달성률·히트맵)에서 빠지고 연속도 거기서 끊긴다.
  줄이는 습관은 빈 주가 "0회 ≤ 한도"라 저절로 달성이 돼, 쓰기 전 주까지 연속으로 세던 문제를 막는다.
- 체크는 탭할 때마다 순환한다 — 늘리는 습관은 **안함 → 했음(✓) → 찬스(★)**,
  줄이는 습관은 **안함 → 지킴(✓) → 함(●) → 찬스(★)**. 줄이는 습관에 **지킴**이 있는 이유는
  체크를 비워두면 "안 했다"인지 "아직 안 눌렀다"인지 알 수 없기 때문 — 지킨 날을 직접 남기면
  오늘 진행률도 마감된다(주간 집계에는 0회로, 한 날만 카운트된다).
- **찬스** — 컨디션 난조·특별한 사정으로 하루를 킵하는 장치.
