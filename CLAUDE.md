...
- **찬스**(#16 완료 · PR #23 로직 · #24 토글/배지 · #25 기타찬스): 체크가 다상태다 —
  늘리기(atLeast)는 안함 → 했음 → 찬스, 줄이기(atMost)는 안함 → **지킴** → 함 → 찬스(`cycleCheck`).
  체크 값은 `true`(했음) · `'kept'`(지킴, `CHECK_KEPT`) ·
  `{ chance: 'weekly'|'monthly'|'bonus', bonusId? }`(찬스).
...
- **지킴**(#46 · atMost 전용): 줄이기 루틴은 비워두면 저절로 달성이라 '안 함'과 '아직 안 눌렀음'이
  구분되지 않는다 → `CHECK_KEPT`(`'kept'`)로 **안 한 날을 적극적으로 기록**한다. 순환의 **첫 단계**다
  (지킨 날이 대부분이라 하루 한 번 탭으로 끝나야 한다). 주간 집계는 그대로 0회지만
  `checkState`가 `'none'`이 아니므로 **오늘 진행 링·리마인더 미완료 수에서는 '처리함'**으로 잡힌다
  (서버 크론도 non-null 셀을 세므로 그대로 동작 — `store.checkedRoutineIds`). 줄이기 루틴의 `'done'`은
  달성이 아니라 '한 날'이라 지킴(틸 ✓)과 **다른 표식**(붉은 ●)으로 그리고, 이름은 뷰가 아니라
  `checkLabel(state, goalType)`이 준다(지킴/함/완료/찬스로 킵함). **스키마 v2 그대로**다 —
  구버전 `sanitizeChecks`는 모르는 값을 그 셀만 버리므로 롤백해도 나머지 기록은 살아 있다.
...
- **백엔드/동기화**(#7 완료 · PR #27·#28·#30·#32): 단독 사용자 전제. `server/`는 **native 의존성 0** Node API다
  ...
  **인증은 직접 구현하지 않는다** — Cloudflare Access가 `routine.chillingdaisy.org/api` 경로에
  ...
  - **로그인 진입점 `GET /api/login`**(#51): Access 뒤에 있으니 미인증이면 엣지가 로그인으로 보내고,
    통과하면 앱 루트로 302한다(목적지 고정 — 쿼리로 받으면 오픈 리다이렉트). 설정의 동기화 섹션에
    같은 출처 **링크**(`<a href="/api/login">`)로 걸려 있다. **fetch로는 로그인을 시작할 수 없다** —
    엣지 302를 따라가 봐야 화면이 없다. 특히 **iOS 홈 화면 앱은 Safari와 쿠키 저장소가 분리**돼
    밖에서 로그인해도 앱 안엔 세션이 없으므로, 앱 안에서 끝나는 이 경로가 유일한 수단이다.
    (SW `navigateFallbackDenylist`에 `/api/`가 있어 이 내비게이션이 앱 셸로 폴백되지 않는다.)
...
- **렌더 성능**(#8): 최적화는 **측정 후에만** 한다. 실측(루틴 5개·과거 10주 체크로 jsdom 벤치)에서
  무거운 건 인라인 스타일 객체가 아니라 **캘린더 재렌더**였다 — 날짜 시트에서 체크 하나 바꿀 때마다
  주 카드 11개(아이콘 수백 개)가 통째로 다시 그려져 토글이 오늘 탭의 4배였다(10.0ms → 4.1ms로 개선).
  구조: 주 카드를 `WeekCard = memo(...)`로 빼고, App의 `weeks` useMemo가 **바뀐 주만 새 객체로**
  내보낸다(`reuseUnchanged` + 압축 시그니처). memo는 참조로 판정하므로 **둘 중 하나만 하면 효과가
  없다** — 매 렌더 새 객체를 만들면 memo가 그대로 뚫린다. 시그니처를 `JSON.stringify(card)`로 만들면
  하루당 루틴 객체까지 직렬화돼 첫 진입이 되레 느려진다(89ms → 121ms) → 그리는 값만 문자열로 압축한다.
  `onOpenDay`는 `setSheetDay`(setState라 참조 고정)를 그대로 넘겨야 한다.
- **색·대비**(#8 다크 대비): 토큰은 `src/index.css`가 원장이고 **용도별로 나뉜다** — 잘못 고르면
  WCAG AA(글자 4.5:1 · UI 경계 3:1)에 걸린다. `--color-primary`(5.45:1)는 surface 위 **글자·아이콘**
  전용이고, **흰 글자를 얹는 배경**은 `--color-primary-strong`(흰 글자 5.47:1)을 쓴다 — primary 위
  흰 글자는 3.03:1로 미달이다. 틸 tint(`--color-primary-50`) **위의 글자**는 `--color-primary-text`
  (5.90:1). `--color-field-border`·`--color-border`는 **장식 전용**(1.8·1.3:1)이라 글자색·상태
  표식으로 쓰면 안 되고, 비활성/부재 글자는 `--color-disabled-text`(4.84:1), 의미 있는 테두리
  (빈 체크 원 = 미완료, 히트맵 '기록 없음')는 `--color-outline`(3.14:1)이다.
  `src/contrast.test.js`가 **index.css에서 값을 읽어** 모든 쌍의 대비를 계산하고, 장식 토큰이
  글자색으로 새어 들어갔는지도 검사한다(색을 하드코딩하면 토큰을 바꿔도 통과해 검사가 죽는다).
- **에러 처리**(#8): `src/ErrorBoundary.jsx`(클래스 컴포넌트)가 `main.jsx`에서 `<App>`을 감싸 렌더/
  ...
  **키보드 조작성**(PR #45): 누를 수 있는 것은 `div`+`onClick`이 아니라 **`<button type="button">`**으로
  둔다 — 탭 포커스·Enter/Space·스크린리더 롤이 공짜로 따라오고, 손으로 `tabIndex`·`onKeyDown`을 흉내 낼
  필요가 없다. 버튼 기본 스타일을 지우므로 `font: inherit`·`text-align: left`를 함께 준다(레이아웃 회귀 방지).
  탭바 현재 탭은 `aria-current="page"`, 상태 표식(체크마크)은 `role="img"` + `checkLabel()` 라벨.
  **브랜딩·메타**(PR #44): `index.html`이 `lang="ko"`이고 `description`·OG·트위터 카드를 갖는다.
  공유 이미지는 별도 배너가 아니라 앱 아이콘 `pwa-512x512.png`(512 정사각)이라 트위터 카드는
  `summary`다(`summary_large_image`는 가로 이미지를 전제라 정사각을 잘라 먹는다). OG/트위터 URL·
  이미지는 **절대 URL**(`https://routine.chillingdaisy.org/`)이라 도메인이 바뀌면 여기도 함께 고친다.
...
- 백로그: ~~#16 루틴별 찬스 시스템~~(완료 · PR #23 로직 · #24 토글/배지 · #25 기타찬스) —
  ...
  ~~#46 줄이기 루틴 '지킴'~~(완료 · PR #47 — 스키마 v2 유지, 값만 추가)
- 운영 후속 **#20**(열림): CI 최적화(PR #21)·머지 브랜치 정리(2026-07-22, 원격·로컬 모두 `main`만
  남음)는 완료. 남은 건 **(선택) `relay.chillingdaisy.org` DNS**뿐 — Cloudflare Zero Trust 대시보드
  작업이라 코드/CI와 무관(사용자 몫)
...
- **다음 → #8 품질**(상시): 유닛·상호작용 테스트 · 에러 바운더리 · 모달/키보드 접근성 · OG 메타는
  머지됐고(PR #39·#40·#42·#44·#45), 남은 건 **(선택) E2E(Playwright) · 성능 점검(불필요한 리렌더) ·
  다크 대비 점검**뿐이다(체크박스는 이슈 #8이 원장) · #20 운영 후속(열림)
