...
서버 푸시로 앱이 꺼져 있어도 알림이 온다. **VAPID 키를 안 넣으면 푸시는 비활성**이고(구독 저장만 되고
발송 안 됨) API는 그대로 뜬다(fail-safe, 크래시 아님). 켜려면:

1. **VAPID 키 1회 생성** (아무 데서나 — 로컬에서 만들어 값만 서버 `.env`로 옮긴다):

   ```bash
   npx --yes web-push generate-vapid-keys
   # Public Key: B... / Private Key: ...  두 값을 복사
   ```

2. **서버 `.env`에 추가** (위 `.env`와 같은 파일, 배포 사용자 소유·0600 유지):

   ```bash
   VAPID_PUBLIC_KEY=<Public Key>
   VAPID_PRIVATE_KEY=<Private Key>
   VAPID_SUBJECT=mailto:you@example.com   # 연락처(mailto: 또는 https URL). 푸시 서비스가 요구.
   ```

   > **PRIVATE 키는 비밀이다** — 커밋 금지(레포 public). `.env`에만 둔다. 공개키는 클라이언트가
   > `/api/push/key`로 받아 구독에 쓴다(민감 아님). 키를 바꾸면 기존 구독은 무효가 되니 재구독 필요.

3. **재기동**: `cd /opt/routine-app && docker compose up -d`.
   확인: 설정 → 클라우드 동기화를 켠 뒤 "잠금 화면 알림 · 이 기기 등록" → "테스트 알림 보내기".
   VAPID 키가 설정돼 있으면 **매일 정시 자동 발송**도 동작한다(#6 2b · API 프로세스 내 1분 크론이
   각 기기의 로컬 `remindHour`에 미완료가 있으면 발송). 키가 없으면 크론은 no-op이다.

### 동기화 데이터의 소유자 키

동기화 행은 이메일이 아니라 Access의 **`sub`(불변 식별자)** 로 묶인다. 이메일은 바뀌거나 다른
사람에게 재할당될 수 있어서, 재할당된 주소로 로그인한 사람이 앞사람 데이터를 읽고 덮어쓸 수 있다.

반대급부로 **로그인 방식(IdP)을 바꾸면 `sub`가 달라져 데이터가 빈 것처럼 보일 수 있다.**
그때는 지워진 게 아니라 다른 키에 묶여 있는 것이므로 재키잉하면 된다:

```bash
# 1) 어느 sub에 얼마나 묶여 있는지 확인
docker exec routine-app-api node src/rekey.js

# 2) 현재 로그인의 sub 확인 — 브라우저로 https://routine.chillingdaisy.org/api/me

# 3) 이관
docker exec routine-app-api node src/rekey.js <옛sub> <새sub>
```

이관은 `owner`만 바꾸는 게 아니라 **`seq`를 새로 발급한다.** 그냥 owner만 바꾸면 행의 `seq`가
옛 커서 이하로 남아, 그 커서를 든 클라이언트가 조회해도 전부 건너뛰어 **데이터가 여전히 없어
보인다.** 손으로 SQL을 쓰지 않고 이 명령을 쓰는 이유이며, 이 규칙은 테스트로 고정돼 있다.
