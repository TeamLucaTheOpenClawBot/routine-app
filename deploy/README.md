...
```bash
docker exec -it routine-app-api node -e "
const {DatabaseSync}=require('node:sqlite');const db=new DatabaseSync('/data/routine.db');
console.log(db.prepare('SELECT owner, COUNT(*) n FROM cells GROUP BY owner').all());"
# 옛 owner와 새 owner를 확인한 뒤
docker exec -it routine-app-api node -e "
const {DatabaseSync}=require('node:sqlite');const db=new DatabaseSync('/data/routine.db');
db.prepare('UPDATE cells SET owner=? WHERE owner=?').run('<새 sub>','<옛 sub>');
db.prepare('UPDATE docs  SET owner=? WHERE owner=?').run('<새 sub>','<옛 sub>');"
```

현재 로그인의 `sub`는 `https://routine.chillingdaisy.org/api/me`로 확인한다.
